...
## 13. Backup complet (archive)

### Créer l'archive

```bash
cd /home/sam
tar -czf websearch_agent.tar.gz \
    --exclude='venv' \
    --exclude='__pycache__' \
    --exclude='.git' \
...
| Inclus | Exclu |
|--------|-------|
| `sources/` (tous les .py) | `venv/` (recréé avec pip) |
| `core/`, `routes/`, `admin/` | `__pycache__/` |
| `backup.sh`, `restore.sh` | `.git/` |
| `requirements.txt`, `Dockerfile` | `node_modules/` |
| `.env` (⚠️ sensible) | `*.log`, `*.db` |
| `docs/migration/` (privé) | |

### Restaurer

```bash
cd /home/sam
tar -xzf websearch_agent.tar.gz
cd websearch_agent
...
# Restaurer les données (depuis un backup backup.sh)
...
```
/home/sam/websearch_agent.tar.gz   ← archive complète
/home/sam/websearch_agent/          ← dossier actif
```

---

*Dernière mise à jour : 21 août 2026 — Ajout backup archive, setup VM dev, Cython, CI/CD*
