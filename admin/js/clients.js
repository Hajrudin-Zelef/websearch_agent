let _loadClientsInFlight = false;
let _loadClientsPending = false;

async function loadClients() {
    if (_loadClientsInFlight) { _loadClientsPending = true; return; }
    _loadClientsInFlight = true;
    try {
        ...
        list.innerHTML = clients.map(c => `
            <div class="client-card ${c.active ? '' : 'inactive'}" data-id="${c.id}">
                ...
                <div class="client-card-actions">
                    ...
                    <button onclick="copyClientKey('${c.id}')" class="btn btn-sm btn-ghost" title="Cle securisee (non recuperable)">
        ...
        _loadClientsInFlight = false;
        if (_loadClientsPending) { _loadClientsPending = false; loadClients(); }
    }
}
...
function refreshLogsPanelIcons() {
    const panel = document.getElementById('logs-panel');
    if (typeof lucide !== 'undefined' && panel) lucide.createIcons({ nodes: [panel] });
}

function buildClientLogsContent(clientName, logs) {
    ...
    let html = `
        ...
        <div class="logs-panel-section" style="flex:1;overflow:hidden;display:flex;flex-direction:column">
...
async function showClientLogs(clientId, clientName) {
        ...
        <div class="logs-panel" id="logs-panel">
            <div class="logs-panel-header">
                <div class="logs-panel-title">
                    ...
                    <div>
                        <h3>${escapeHtml(clientName)}</h3>
                        <span class="logs-panel-subtitle">Chargement des requetes...</span>
                    </div>
                </div>
    ...
    try {
        ...
        const panel = document.getElementById('logs-panel');
        if (panel) {
            panel.innerHTML = buildClientLogsContent(clientName, logs);
            refreshLogsPanelIcons();
        }
    } catch (e) {
        const panel = document.getElementById('logs-panel');
            ...
            panel.innerHTML = `
                <div class="logs-panel-header">
                    <div class="logs-panel-title">
                        ...
                        <div>
                            <h3>${escapeHtml(clientName)}</h3>
                            <span class="logs-panel-subtitle">Erreur</span>
                        </div>
                    </div>
                    <button class="btn btn-sm btn-ghost" onclick="closeLogsPanel()"><i data-lucide="x"></i></button>
                </div>
                <div class="logs-empty-state">
                    <i data-lucide="wifi-off" style="width:40px;height:40px;color:var(--text-faint)"></i>
                    <p>Impossible de charger les requetes.</p>
                </div>`;
            refreshLogsPanelIcons();
        }
...
async function copyClientKey(clientId) {
    toast('Pour des raisons de securite, la cle ne peut plus etre affichee apres creation. Utilisez « Regenerer » pour en obtenir une nouvelle.', 'warning');
}
...
async function deactivateClient(clientId) {
    ...
    const card = $(`.client-card[data-id="${clientId}"]`);
    if (card) { card.classList.add('inactive'); card.style.pointerEvents = 'none'; }
    try {
...
async function activateClient(clientId) {
    const card = $(`.client-card[data-id="${clientId}"]`);
    if (card) { card.classList.remove('inactive'); card.style.pointerEvents = 'none'; }
    try {
        await api(`/admin/clients/${clientId}/activate`, { method: 'POST' });
        toast('App activee', 'success');
        loadClients();
    } catch (e) {
        toast('Erreur', 'error');
        loadClients();
